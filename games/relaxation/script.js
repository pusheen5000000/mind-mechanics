(() => {
  ...
  function setToolboxX(x) {
    ...
    toolboxX = Math.max(0, Math.min(x, stageWidth - TOOLBOX_WIDTH));
  ...
  function handlePointerMove(clientX) {
    ...
    setToolboxX(clientX - rect.left - TOOLBOX_WIDTH / 2);
  }
  ...
  // ===== Relaxation break (triggered by catching a sparkle) =====
  function triggerRelaxation() {
    ...
    const exercise =
      RELAX_EXERCISES[Math.floor(Math.random() * RELAX_EXERCISES.length)];
  ...
  function gameLoop() {
    if (!isPaused) {
      ...
      for (let i = fallingItems.length - 1; i >= 0; i--) {
        ...
        const reachedToolbox = itemRect.bottom > toolboxRect.top;

        if (overlapsX && reachedToolbox && item.y < stageHeight) {
          item.el.remove();
